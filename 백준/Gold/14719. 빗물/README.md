...
### 출력 
 ...
 ### 풀이 방법
1. 처음에는 앞에서 큰거 잡고 뒤 쪽에서 큰거 잡고 그 사이 값을 계산 하려고 함
   - 문제는 뒤 쪽에서는 흐름이 반대가 되서 적용이 안되는 문제가 있었음
  
2. 배열을 순회하면서 그 값에 대한 고이는 값을 계산할 수 있도록 바꿈
3. 해당 idx을 기준으로 왼쪽에서 가장 큰 값, 오른쪽에서 가장 큰 값 그 둘 중 더 작은 값을 기준으로 얼마나 쌓이는지 보면됨
4. 생각하기 어려운 문제였음 ⭐⭐⭐
